"""Build LLM-verified user profiles (fact bank v2) — see utils/user_profiles.py.
...
def main() -> int:
    ...
    def progress(author, slot, verdict):
        ...
        print(f"[{calls['n']:>4}] {tag} {author}/{slot}: "
              f"{verdict.get('value')!r} ({verdict.get('sincerity')}) "
              f"<- {verdict['content'][:70]}")

    store = user_profiles.build_profiles(authors, per_slot_cap=args.cap,
                                         progress=progress)
    print(f"\nJudge calls this run: {calls['n']}")
    for author in authors:
